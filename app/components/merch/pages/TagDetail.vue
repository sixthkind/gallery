...
const fetchTagsList = async () => {
  try {
    tags.value = await pb.collection(MERCH_COLLECTIONS.tags).getFullList({
      sort: 'name'
    });
  } catch (error) {
...
const queueTagSave = () => {
  ...
  saveTimer.value = setTimeout(async () => {
    ...
    try {
      tagRecord.value = await pb.collection(MERCH_COLLECTIONS.tags).update(tagRecord.value.id, {
        name: newName
      });
      ...
      if (newName !== tagName.value) {
        window.history.replaceState({}, '', toMerchPath(`/tags/${encodeURIComponent(newName)}`));
      }
    } catch (error) {
    ...
    buttons: [
      ...
      },
      {
        ...
        handler: async () => {
          ...
          try {
            await pb.collection(MERCH_COLLECTIONS.tags).delete(tagRecord.value.id);
            router.push({ path: toMerchPath("/tags"), query: { refreshed: Date.now().toString() } });
          } catch (error) {
