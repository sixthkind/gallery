...
const { showUpload } = useMerchState();
...
const uploadPhotos = async () => {
  ...
  if (props.albumId) {
    try {
      const album = await pb.collection(MERCH_COLLECTIONS.albums).getOne(props.albumId);
      if (!album.coverPhoto) {
        shouldSetCover = true;
      } else {
  ...
  for (const fileObj of selectedFiles.value) {
    ...
    try {
      ...
      const createdPhoto = await pb.collection(MERCH_COLLECTIONS.photos).create(formData);
      if (props.albumId && shouldSetCover && !albumCoverId) {
        try {
          await pb.collection(MERCH_COLLECTIONS.albums).update(props.albumId, { coverPhoto: createdPhoto.id });
  ...
  // Emit event to refresh merch if all uploads succeeded
