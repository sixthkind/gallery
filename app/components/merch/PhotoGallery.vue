<template>
  <div class="photo-merch-wrapper">
    ...
    <!-- Loading State -->
    <div v-if="loading">
      <MerchPhotoSkeletonGrid :layout="props.currentLayout" />
    </div>
    ...
    <!-- Merch Content -->
    <div v-else class="merch-grid-shell">
      <div
        v-if="expandedGroups.size > 0"
        class="merch-dismiss-zone merch-dismiss-zone--top"
    ...
    >
          ...
          <div class="group-stack-container">
            ...
            >
              <img
                ...
                :class="[
                  'w-full',
                  props.currentLayout === 'tile' ? 'h-full object-contain bg-slate-100 dark:bg-slate-900/40' : 'h-auto object-cover'
                ]"
                loading="lazy"
              />
            </div>
      ...
      <div
        v-if="expandedGroups.size > 0"
        class="merch-dismiss-zone merch-dismiss-zone--bottom"
        @click="collapseAllGroups"
        aria-hidden="true"
      ></div>
    </div>
...
// Use shared merch state
const { selectedPhotos } = useMerchState();
...
const ensureGroupPhotoSortOrder = async () => {
  ...
  await Promise.all(updates.map(async ({ groupId, photoId, sortOrder }) => {
    try {
      await pb.collection(MERCH_COLLECTIONS.photos).update(photoId, { sortOrder });
...
const baseItems = computed(() => {
  ...
  const addGroupItem = (group) => {
    ...
    items.push({
      ...
      description: group.shortDescription || group.description || '',
...
// All photos for lightbox navigation (follows merch order)
...
const toggleFavorite = async (item) => {
  ...
  if (item.isGroup) {
    ...
    try {
      await pb.collection(MERCH_COLLECTIONS.groups).update(item.id, { favorite: nextFavorite });
    } catch (error) {
  ...
  try {
    await pb.collection(MERCH_COLLECTIONS.photos).update(item.id, { favorite: nextFavorite });
  } catch (error) {
...
const reorderItems = async ({ sourceId, targetId, groupId }) => {
  ...
  if (groupId) {
    ...
    try {
      await pb.collection(MERCH_COLLECTIONS.photos).update(sourceId, { sortOrder: newOrder });
...
const replaceSelectedPhoto = async (file) => {
  ...
  try {
    ...
    await pb.collection(MERCH_COLLECTIONS.photos).update(target.id, formData);
    refresh();
  } catch (error) {
...
const createQuickGroup = async () => {
  ...
  try {
    ...
    const productName = String(firstPhoto?.title || '').trim() || `Product ${Date.now()}`;
    const groupData = {
      name: productName,
      slug: `product-${Date.now()}`,
      shortDescription: String(firstPhoto?.description || '').trim(),
      details: '',
      price: 0,
      currency: 'USD',
      active: true,
      title: productName,
      description: String(firstPhoto?.description || '').trim(),
      coverPhoto: coverPhotoId,
      photos: photoIds,
      sortOrder: groupSortOrder
    };
    if (currentUser.collectionName === 'users') {
      groupData.user = currentUser.id;
    }
    if (props.albumId) {
      groupData.album = props.albumId;
    }

    const group = await pb.collection(MERCH_COLLECTIONS.groups).create(groupData);

    for (const photoId of photoIds) {
      const updateData = { product: group.id };
      if (props.albumId) {
        updateData.album = props.albumId;
      }
      await pb.collection(MERCH_COLLECTIONS.photos).update(photoId, updateData);
    }
...
const openProductFromGroup = async (groupId) => {
  if (!groupId) return false;

  let groupRecord = groups.value.find(group => group.id === groupId);
  if (!groupRecord) {
    try {
      groupRecord = await pb.collection(MERCH_COLLECTIONS.groups).getOne(groupId);
    } catch (error) {
      return false;
    }
  }

  let slug = String(groupRecord?.slug || '').trim();
  if (!slug) {
    const rawName = String(groupRecord?.name || groupRecord?.title || '').trim().toLowerCase();
    const normalizedName = rawName
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-+|-+$/g, '');
    const fallbackSlug = normalizedName || `product-${groupId}`;

    try {
      await pb.collection(MERCH_COLLECTIONS.groups).update(groupId, { slug: fallbackSlug });
      slug = fallbackSlug;
    } catch (error) {
      return false;
    }
  ...
  await navigateTo(toMerchPath(`/products/${encodeURIComponent(slug)}`));
  return true;
};
...
const handleItemClick = async (item) => {
  if (item.isGroup) {
    ...
    if (item.isGroupPhoto && item.parentGroupId) {
      const opened = await openProductFromGroup(item.parentGroupId);
      if (opened) return;
    }
...
const deletePhoto = async (photo) => {
  ...
  try {
    ...
    if (photo.product) {
      groupToCheck = photo.product;
      const group = await pb.collection(MERCH_COLLECTIONS.groups).getOne(photo.product);
      const updatedPhotos = group.photos.filter(id => id !== photo.id);
      
      // Update group photos
      await pb.collection(MERCH_COLLECTIONS.groups).update(group.id, {
        photos: updatedPhotos
      });
    ...
    await pb.collection(MERCH_COLLECTIONS.photos).delete(photo.id);
    photos.value = photos.value.filter(p => p.id !== photo.id);
    
    // Check if the group has 0 or 1 photos left and handle accordingly
    if (groupToCheck) {
      try {
        const group = await pb.collection(MERCH_COLLECTIONS.groups).getOne(groupToCheck);
        if (!group.photos || group.photos.length === 0) {
          // Delete empty group
          await pb.collection(MERCH_COLLECTIONS.groups).delete(groupToCheck);
          console.log(`Automatically deleted empty product: ${groupToCheck}`);
          ...
          await pb.collection(MERCH_COLLECTIONS.photos).update(lastPhotoId, { 
            product: '',
            sortOrder: groupSortOrder
          });
          ...
          // Delete the group
          await pb.collection(MERCH_COLLECTIONS.groups).delete(groupToCheck);
          console.log(`Automatically deleted single-photo product: ${groupToCheck}, photo ${lastPhotoId} is now standalone`);
        }
      } catch (error) {
...
const updateSelectedLocations = async (location) => {
  ...
  try {
    await Promise.all(photoIds.map(photoId => {
      return pb.collection(MERCH_COLLECTIONS.photos).update(photoId, { location });
    }));
...
const deleteSelectedPhotos = async () => {
  ...
  try {
    ...
    for (const photo of photosToDelete) {
      // If photo is in a group, remove it from the group first
      if (photo.product) {
        groupsToCheck.add(photo.product); // Track this group
        const group = await pb.collection(MERCH_COLLECTIONS.groups).getOne(photo.product);
        const updatedPhotos = group.photos.filter(id => id !== photo.id);
    ...
    for (const groupId of groupsToCheck) {
      try {
        const group = await pb.collection(MERCH_COLLECTIONS.groups).getOne(groupId);
        if (!group.photos || group.photos.length === 0) {
          // Delete empty group
          await pb.collection(MERCH_COLLECTIONS.groups).delete(groupId);
          console.log(`Automatically deleted empty product: ${groupId}`);
        } else if (group.photos.length === 1) {
          // Only one photo left - convert to standalone photo
          const lastPhotoId = group.photos[0];
          const groupSortOrder = group.sortOrder;
          ...
          // Delete the group
          await pb.collection(MERCH_COLLECTIONS.groups).delete(groupId);
          console.log(`Automatically deleted single-photo product: ${groupId}, photo ${lastPhotoId} is now standalone`);
        }
      } catch (error) {
...
const addPhotosToGroup = async () => {
  ...
  try {
    ...
    // Update group photos
    await pb.collection(MERCH_COLLECTIONS.groups).update(groupId, {
      photos: updatedPhotos
    });
    
    // Update the photos to add the group reference
    for (const photoId of selectedPhotos.value) {
      const updateData = { product: groupId };
      if (group.album) {
  ...
  try {
    ...
    if (updatedPhotos.length === 0) {
      ...
      for (let i = 0; i < selectedPhotos.value.length; i++) {
        ...
        await pb.collection(MERCH_COLLECTIONS.photos).update(photoId, {
          product: '',
...
const confirmDeleteGroup = async () => {
  ...
  const alert = await alertController.create({
    header: 'Delete Group',
    message: `Are you sure you want to delete the product "${group.name || group.title || 'Untitled Product'}"? This will remove all photos from the product and delete it.`,
    buttons: [
      {
        text: 'Cancel',
        role: 'cancel'
...
// Refresh merch (called from parent)
const refresh = () => {
  loading.value = true;
  fetchPhotos();
};
