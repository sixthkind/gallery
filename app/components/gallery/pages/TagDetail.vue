  ...
  saveTimer.value = setTimeout(async () => {
    ...
    try {
      ...
      if (newName !== tagName.value) {
        window.history.replaceState({}, '', toGalleryPath(`/tags/${encodeURIComponent(newName)}`));
      }
    } catch (error) {
...
const goBack = () => {
  ...
  router.push(toGalleryPath("/tags"));
};
      ...
      },
      {
        ...
        handler: async () => {
          ...
          try {
            ...
            router.push({ path: toGalleryPath("/tags"), query: { refreshed: Date.now().toString() } });
          } catch (error) {
        ...
        <div class="flex items-center justify-between mt-4 mb-6">
          ...
          <div class="flex items-center gap-3">
            ...
            <button
              ...
              @click="router.push(toGalleryPath(`/tags/${encodeURIComponent(nextTag.name)}`))"
              aria-label="Next tag"
            >
